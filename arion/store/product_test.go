	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Product A", "A great product", 1000, fixedTime, nil)
				mock.ExpectQuery(`SELECT id, name, description, price, created_at, updated_at\s+FROM products\s+WHERE id = \$1`).
					WithArgs(1).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Product A", "A great product", 1000, fixedTime, nil)
				mock.ExpectQuery(`SELECT id, name, description, price, created_at, updated_at\s+FROM products\s+WHERE id = \$1\s+AND shop_id = \$2`).
					WithArgs(1, 10).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
...
func Test_product_GetProductsByShopID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Product A", "Description A", 1000, fixedTime, nil).
					AddRow(2, "Product B", "Description B", 2000, fixedTime, nil)
				mock.ExpectQuery(`SELECT id, name, description, price, created_at, updated_at\s+FROM products\s+WHERE shop_id = \$1`).
					WithArgs(10).
					WillReturnRows(rows)
			},
			wantResult: []model.Product{
				{ID: 1, Name: "Product A", Description: "Description A", Price: 1000, CreatedAt: fixedTime},
				{ID: 2, Name: "Product B", Description: "Description B", Price: 2000, CreatedAt: fixedTime},
			},
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, gotErr := store.GetProductsByShopID(tt.shopID)
...
func Test_product_CreateProduct(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(`INSERT INTO products \(name, description, price, shop_id, created_at\)\s+VALUES \(\$1, COALESCE\(\$2, ''\), \$3, \$4, \$5\)\s+RETURNING id, description`).
					WithArgs("New Product", strPtr("Product description"), 1500, 10, sqlmock.AnyArg()).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
				ID:          1,
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(`INSERT INTO products \(name, description, price, shop_id, created_at\)\s+VALUES \(\$1, COALESCE\(\$2, ''\), \$3, \$4, \$5\)\s+RETURNING id, description`).
					WithArgs("Existing Product", nil, 1000, 10, sqlmock.AnyArg()).
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(`INSERT INTO products \(name, description, price, shop_id, created_at\)\s+VALUES \(\$1, COALESCE\(\$2, ''\), \$3, \$4, \$5\)\s+RETURNING id, description`).
					WithArgs("New Product", nil, 1000, 10, sqlmock.AnyArg()).
					WillReturnError(errors.New("database error"))
			},
			wantResult: nil,
			wantErr:    true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, gotErr := store.CreateProduct(tt.input.name, tt.input.description, tt.input.price, tt.input.shopID)
...
func Test_product_UpdateProduct(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Updated Product", "Description", 1000, fixedTime, updatedTime)
				mock.ExpectQuery(`UPDATE products\s+SET name = 'Updated Product',updated_at = now\(\)\s+WHERE id = \$1\s+RETURNING id, name, description, price, created_at, updated_at`).
					WithArgs(1).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Product A", "Description", 2000, fixedTime, updatedTime)
				mock.ExpectQuery(`UPDATE products\s+SET price = 2000,updated_at = now\(\)\s+WHERE id = \$1\s+RETURNING id, name, description, price, created_at, updated_at`).
					WithArgs(1).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "name", "description", "price", "created_at", "updated_at"}).
					AddRow(1, "Product A", "Updated description", 1000, fixedTime, updatedTime)
				mock.ExpectQuery(`UPDATE products\s+SET description = 'Updated description',updated_at = now\(\)\s+WHERE id = \$1\s+RETURNING id, name, description, price, created_at, updated_at`).
					WithArgs(1).
					WillReturnRows(rows)
			},
			wantResult: &model.Product{
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(`UPDATE products\s+SET name = 'Existing Name',updated_at = now\(\)\s+WHERE id = \$1\s+RETURNING id, name, description, price, created_at, updated_at`).
					WithArgs(1).
					WillReturnError(&pq.Error{Code: "23505"})
			},
			wantResult: nil,
		...
		},
		{
			...
			mockSetup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(`UPDATE products\s+SET name = 'New Name',updated_at = now\(\)\s+WHERE id = \$1\s+RETURNING id, name, description, price, created_at, updated_at`).
					WithArgs(9999).
					WillReturnError(sql.ErrNoRows)
			},
			wantResult: nil,
